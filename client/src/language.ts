...
(() => {
  ...
  const codes: Record<Code, Locale> = { EN: 'en', ZH: 'zh-TW', JA: 'ja', KO: 'ko' };
  const byCode: Partial<Record<string, Locale>> = codes;
  const codeFor = (language: string): Code =>
    (Object.keys(codes) as Code[]).find(code => codes[code] === language) || 'EN';
  const parseLanguage = (value: string | null): Locale | null => {
  ...
  function publicURL(value: string, locale: string = language) {
    const url = new URL(value, location.href);
    const match = url.pathname.match(editionPath);
    if (url.origin !== location.origin || !match) return url;
    url.pathname = `${match[1]}index.html`;
    url.searchParams.set('lang', codeFor(locale));
    return url;
  }
  function sourceURL(locale: string) {
    const url = new URL(location.href);
    const match = url.pathname.match(editionPath);
    if (!match) throw Error('Unsupported edition path');
  ...
  function updateLinks() {
    ...
    document.querySelectorAll<HTMLFormElement>('form[role="search"]').forEach(form => {
      ...
      let field = form.querySelector<HTMLInputElement>('input[name="lang"]');
      if (!field) {
  ...
  function notice(message = '') {
    let status = document.querySelector<HTMLElement>('[data-language-status]');
    if (!status && message) {
  ...
  const messages: Record<Locale, [string, string]> = {
  ...
  function key(element: Element): string {
    ...
    const prefix = article ? `#${CSS.escape(article.id)}` : preview
      ? `[data-preview-post-id="${CSS.escape((preview as HTMLElement).dataset.previewPostId!)}"]` : 'body';
  ...
  function capture(): Position {
    ...
    return {
      ...
      details: [...document.querySelectorAll<HTMLDetailsElement>('main details')]
        .map((element): [string, boolean] => [key(element), element.open]),
      query: document.querySelector<HTMLInputElement>('#story-search')?.value,
    };
  }
  function updateHead(next: Document, locale: string) {
    ...
    ['meta[name="description"]', 'meta[property="og:title"]', 'meta[property="og:description"]',
      ...
      if (selector.includes('canonical') || selector.includes('og:url')) value = publicURL(value, locale).href;
      current.setAttribute(name, value);
    });
  }
  function restore(position: Position) {
    position.details.forEach(([selector, open]) => {
      const element = document.querySelector<HTMLDetailsElement>(selector);
      if (element?.tagName === 'DETAILS') element.open = open;
    });
    let anchor = position.anchor && document.querySelector(position.anchor);
    if (anchor && !anchor.getClientRects().length) anchor = null;
  ...
  async function change(locale: string | undefined,
                        { initial = false, history = true }: { initial?: boolean; history?: boolean } = {}) {
    if (!locale || !(Object.values(codes) as string[]).includes(locale)) return;
    ...
    try {
      ...
      if (!response.ok) throw Error(`Language response ${response.status}`);
      const next = new DOMParser().parseFromString(await response.text(), 'text/html');
      if (token !== revision) return;
      if (next.documentElement.lang !== locale || !next.querySelector('main')) throw Error('Invalid language page');
      ...
      try {
        ...
        if (search && position.query !== undefined) search.value = position.query;
        updateLinks();
  ...
  document.addEventListener('click', event => {
    const link = event.target instanceof Element
      ? event.target.closest<HTMLAnchorElement>('a[data-language]') : null;
    if (link) refreshLanguageLinks();
