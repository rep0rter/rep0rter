...
(() => {
  interface Reveal { x: number; y: number; radius: number; duration: number; }
  interface Cell {
    cell: HTMLSpanElement;
    text: HTMLSpanElement;
    rune: HTMLSpanElement;
    rect: DOMRect;
    delay: number;
    next: number;
    node: Text;
  ...
  const visibleRect = (rect: DOMRect) => rect.width > 0 && rect.height > 0 && rect.bottom > 0
  ...
  function arrival(fraction: number) {
    let lower = 0;
    let upper = 1;
  ...
  function play(element: HTMLElement = document.body, reveal: Reveal | null = null): () => void {
    ...
    try {
      ...
      let node: Text | null;
      while (cells.length < glyphLimit && (node = walker.nextNode() as Text | null)) {
      ...
      cells.forEach((cell, index) => {
        ...
        const options: KeyframeAnimationOptions = { duration, delay: cell.delay, fill: 'both', easing: 'linear' };
  ...
  document.addEventListener('rep0rter:theme-reveal', event => {
    ...
    if (!finite(x) || !finite(y) || !finite(radius) || !finite(revealDuration) || radius <= 0) { cancel(); return; }
  ...
  document.addEventListener('pointerdown', event => {
    ...
    if (window.Rep0rterScrollLock?.isLocked()
      && (event.pointerType === 'touch'
        || !(event.target as Element | null)?.closest?.('a,button,summary,input,textarea,select,[contenteditable]'))) return;
    cancel();
