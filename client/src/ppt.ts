...
(() => {
  const stage = document.querySelector<HTMLElement>('.ppt-stage');
  const slides = Array.from(document.querySelectorAll<HTMLElement>('.ppt-slide'));
  if (!stage || !slides.length) return;
  const dots = Array.from(document.querySelectorAll<HTMLButtonElement>('[data-slide-to]'));
  ...
  const counter = document.querySelector<HTMLElement>('[data-slide-number]');
  const status = document.querySelector<HTMLElement>('[data-slide-status]');
  let current = -1;
  const fromHash = () => {
  ...
  const frame = demoSlide?.querySelector<HTMLIFrameElement>('iframe[data-src]');
  const browser = frame?.closest<HTMLElement>('.ppt-browser');
  ...
  const prepareEmbeddedNavigation = () => {
    try {
      ...
      if (!doc?.head || doc.getElementById('ppt-embedded-navigation')) return;
  ...
  const announce = () => {
    if (status) status.textContent = `Slide ${current + 1} of ${slides.length}: ${slides[current]?.dataset.label || ''}${current === 2 ? `. Demo ${demoStep + 1} of 4: ${demoLabels[demoStep]}` : ''}`;
  ...
  const applyDemo = () => {
    if (!frame || !demoSlide || current !== 2) return;
    const edition = demoStep >= 2 ? 'ja' : 'en';
    demoSlide.dataset.demoStep = String(demoStep);
    const label = demoSlide.querySelector<HTMLElement>('[data-demo-label]');
    const count = demoSlide.querySelector<HTMLElement>('[data-demo-count]');
    const caption = demoSlide.querySelector<HTMLElement>('[data-demo-caption]');
    if (label) label.textContent = demoLabels[demoStep] || '';
    if (count) count.textContent = `${String(demoStep + 1).padStart(2, '0')} / 04`;
    if (caption) caption.textContent = demoCaptions[demoStep] || '';
    ...
    try {
      ...
      const filters = doc.querySelector<HTMLDetailsElement>('[data-filters]');
      if (filters) filters.open = demoStep === 1;
      const source = doc.querySelector<HTMLSelectElement>('[data-filter="source"]');
      if (source) {
      ...
      const anchor = demoStep === 1 ? filters : doc.querySelector<HTMLElement>('#news');
      if (anchor) win.scrollTo({ top: Math.max(0, anchor.getBoundingClientRect().top + win.scrollY - 130), behavior: 'instant' });
      ...
      const loading = demoSlide.querySelector<HTMLElement>('.ppt-preview-loading');
      if (loading) loading.textContent = 'The live preview is unavailable. Continue with →.';
      browser?.setAttribute('aria-busy', 'false');
    }
  };
  frame?.addEventListener('load', () => {
    try { loadedEdition = frame.contentDocument?.documentElement.lang || ''; } catch { loadedEdition = ''; }
    prepareEmbeddedNavigation();
    if (current !== 2) return;
    ...
    if (loadedEdition !== desired) {
      ...
      try {
        if (frame.contentWindow?.location.pathname === new URL(demoURL(desired)).pathname) {
          const loading = demoSlide?.querySelector<HTMLElement>('.ppt-preview-loading');
          if (loading) loading.textContent = 'This edition is unavailable. Continue with →.';
          browser?.setAttribute('aria-busy', 'false');
        }
  ...
  const go = (requested: number, updateHash = true, entryStep = 0) => {
    const index = Math.min(slides.length - 1, Math.max(0, requested));
    if (index === current && (index !== 2 || demoStep === entryStep)) return;
    ...
    current = index;
    if (current === 2) demoStep = entryStep;
    slides.forEach((slide, position) => {
      ...
      slide.inert = !active;
      if (active) { slide.scrollTop = 0; if (position === 2) applyDemo(); }
    });
  ...
  document.addEventListener('keydown', event => {
    ...
    switch (event.key) {
      case 'ArrowRight': case 'PageDown': event.preventDefault(); advance(1); return;
      case 'ArrowLeft': case 'PageUp': event.preventDefault(); advance(-1); return;
  ...
  stage.addEventListener('touchend', event => {
    ...
    touchStart = null;
    if (Math.abs(dx) > 65 && Math.abs(dx) > Math.abs(dy) * 1.6) advance(dx < 0 ? 1 : -1);
  }, { passive: true });
