...
(() => {
  ...
  const tick = (now: number) => {
    ...
    for (const motion of [...active]) {
      ...
      motion.velocity = decay * (
        -20 * (displacement * cosine + slope * sine) +
  ...
  const spring = ({ from, to, velocity = 0, onUpdate, onComplete }: SpringOptions): SpringHandle => {
    const motion: Motion = {
      value: from, to, velocity, onUpdate, onComplete,
      tolerance: Math.max(Math.abs(to - from) * 0.001, 0.0001),
    };
    const handle: SpringHandle = { cancel: () => {
      active.delete(motion);
  ...
  reduced.addEventListener('change', () => {
    ...
    frame = 0;
    for (const motion of [...active]) {
      if (active.has(motion)) finish(motion);
    }
  });
  ...
  const initialize = () => {
    ...
    let pendingPointer: { target: HTMLElement | null; x: number; y: number } | null = null;
    let pointerFrame = 0;
    ...
    const paintPointer = () => {
      ...
      if (target !== highlight) clearHighlight();
      if (!target?.isConnected) return;
      highlight = target;
      const bounds = target.getBoundingClientRect();
      const percentage = (point: number, origin: number, size: number) => `${Math.min(100, Math.max(0, (point - origin) / Math.max(size, 1) * 100)).toFixed(2)}%`;
    ...
    document.addEventListener('pointermove', event => {
      ...
      pendingPointer = {
        target: event.target instanceof Element ? event.target.closest<HTMLElement>(surfaces) : null,
