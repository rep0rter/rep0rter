...
每輪 `sources.<source>.metrics` 分別保存各來源的六項計數，包括失敗與重試的請求。
`pages` 是收到的 HTTP 回應數（含錯誤回應）；連線失敗只增加 requests。
`acceptance-report` 的 `collection.by_source` 與 `weekly_comparison` 可比較各來源最近七天及前七天的
實測總量、每輪平均請求、日期數與 degraded 輪數。七天區間以報表時間為右界，採 `(起點, 終點]`。
歷史輪次若沒有完整的來源計數，列入 unmeasured，不從全站總量推算 Slack 用量；沒有實測時總量與
平均為 null，空區間沒有來源項目。前後兩週的來源與流量可能不同，不能把描述性比較當作受控改善率。
未知漏事件數仍為 null，需另有上游基準才能判斷；各來源的兩週量測狀態與全站觀測期分開呈現。

補抓缺少的主文若未回傳精確 ID，仍屬未解決缺口；六小時退避期間、補抓額度用完或待處理新回覆
都不會讓已知缺口變成健康。成功取得並保存主文後才清除缺口，沿用來源退出及公開頻道限制。
