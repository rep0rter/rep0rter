...
(() => {
    ...
    const revealFrom = (trigger) => {
        ...
        try {
            ...
            transition.ready.then(() => {
                if (activeReveal !== reveal || revision !== reveal.revision)
                    return;
                try {
    ...
    const initializeControls = () => {
        ...
        menus().forEach(menu => {
            ...
            const trigger = menu.querySelector('summary');
            if (bound.has(trigger))
                return;
            bound.add(trigger);
            trigger.addEventListener('click', () => {
                menus().forEach(other => { if (other !== menu)
                    other.open = false; });
            });
        });
        apply();
    ...
    document.addEventListener('pointerdown', event => {
        menus().forEach(menu => { if (!menu.contains(event.target))
            menu.open = false; });
    });
