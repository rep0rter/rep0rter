...
(() => {
  ...
  const parseLanguage = value => {
    ...
    return codes[code] || (code === 'ZH-TW' ? 'zh-TW' : null);
  };
  ...
  function capture() {
    ...
    return {
      ...
      details: [...document.querySelectorAll('main details')].map(element => [key(element), element.open]),
  ...
  async function change(locale, { initial = false, history = true } = {}) {
    if (!Object.values(codes).includes(locale)) return;
    ...
    try {
      ...
      try {
        ...
        restore(position);
        if (!initial) document.querySelector('.language-trigger')?.focus({ preventScroll: true });
        notice();
      } finally {
  ...
  document.addEventListener('click', event => {
    const link = event.target instanceof Element ? event.target.closest('a[data-language]') : null;
