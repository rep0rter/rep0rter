...
(() => {
  ...
  const initialize = () => {
    ...
    let touch = null;
    const axes = { x: { value: 0, velocity: 0 }, y: { value: 0, velocity: 0 }, light: { value: 0, velocity: 0 } };
    ...
    const syncImages = () => {
      ...
      document.querySelectorAll('[data-image-light][data-image-dark]').forEach(element => {
        ...
        if (element.matches('a')) { element.href = src; return; }
    ...
    const point = event => {
      if (!card || reduced.matches || closing || viewer.dataset.imageState !== 'ready') return;
