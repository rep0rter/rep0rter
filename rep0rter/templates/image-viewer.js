...
(() => {
  ...
  viewer.addEventListener('close', () => { if (!viewer.open) cleanup(); });
  ...
  toolbar.addEventListener('pointerup', event => release(event));
  toolbar.addEventListener('pointercancel', event => release(event, true));
  toolbar.addEventListener('lostpointercapture', event => release(event, true));
})();
