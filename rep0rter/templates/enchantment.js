...
(() => {
    ...
    function play(element = document.body, reveal = null) {
        cancel();
        if (!(element instanceof HTMLElement) || !allowed() || selected() || !element.isConnected)
            return () => { };
        ...
        try {
                ...
                acceptNode(node) {
                    ...
                    if (!accepted.has(parent)) {
                        ...
                        if (typeof parent.checkVisibility === 'function'
                            ...
                            eligible = false;
                        if (!visibleRect(parent.getBoundingClientRect()))
                            eligible = false;
            ...
            while (cells.length < glyphLimit && (node = walker.nextNode())) {
                // acceptNode rejected every node without a parent element.
                const parentElement = node.parentElement;
                const computed = styles.get(parentElement);
                const nodeRange = document.createRange();
                nodeRange.selectNodeContents(node);
                if (![...nodeRange.getClientRects()].some(visibleRect))
                    continue;
                for (const part of segmenter.segment(node.data)) {
                    if (cells.length >= glyphLimit)
                        break;
                    ...
                    if (header && headerRect && !header.contains(parentElement)
                        && rect.top < headerRect.bottom && rect.bottom > headerRect.top
                        && rect.left < headerRect.right && rect.right > headerRect.left)
                        continue;
                    const covering = document.elementFromPoint(Math.min(innerWidth - 1, Math.max(0, rect.left + rect.width / 2)), Math.min(innerHeight - 1, Math.max(0, rect.top + rect.height / 2)));
                    const parent = parentElement;
                    if (covering && !parent.contains(covering) && !covering.contains(parent))
                        continue;
                    ...
                    layer.append(cell);
                    const distance = reveal ? Math.hypot(Math.max(rect.left - reveal.x, 0, reveal.x - rect.right), Math.max(rect.top - reveal.y, 0, reveal.y - rect.bottom)) : 0;
                    const delay = reveal ? arrival(Math.min(1, distance / reveal.radius)) * reveal.duration + 16 : 0;
                    // resolveAt is assigned once every cell exists and the list length is known.
                    cells.push({ cell, text, rune, rect, delay, next: delay + 70 + Math.random() * 40, node, resolveAt: 0 });
                    ranges.push(range);
                }
            }
            if (!cells.length) {
            ...
            const tick = (now) => {
                if (current !== revision || !element.isConnected || !allowed() || now - started >= end) {
                    stop();
                    return;
                }
    ...
    document.addEventListener('rep0rter:theme-reveal', event => {
        ...
        if (!finite(x) || !finite(y) || !finite(radius) || !finite(revealDuration) || radius <= 0) {
            cancel();
            return;
        }
    ...
    for (const event of ['pagehide', 'resize', 'scroll'])
    ...
    document.addEventListener('pointerdown', event => {
        ...
        if (window.Rep0rterScrollLock?.isLocked()
            && (event.pointerType === 'touch'
                || !event.target?.closest?.('a,button,summary,input,textarea,select,[contenteditable]')))
            return;
        cancel();
    }, { passive: true, capture: true });
    document.addEventListener('click', cancel, { capture: true });
    document.addEventListener('keydown', event => {
        if (window.Rep0rterScrollLock?.isLocked() && window.Rep0rterScrollLock.isScrollKey(event))
            return;
        cancel();
    }, { capture: true });
    document.addEventListener('selectionchange', () => { if (selected())
        cancel(); });
