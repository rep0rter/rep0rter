...
`worker.js` reproduces `Caddyfile`: `/auth/*`, `/submit`, `/projects` and
...
`tests/test_cloudflare_deploy.py` guards both, and checks that the forwarded
paths still match the `Caddyfile` matcher.
...
Run the Worker against a locally built site, and point `APP_ORIGIN` at a
non-production host so the check does not send traffic to the live app:

```sh
npx wrangler dev --config deploy/cloudflare/wrangler.jsonc \
  --assets "${REP0RTER_DATA_DIR:-./data}/site" --local --var APP_ORIGIN:http://127.0.0.1:8000
```
