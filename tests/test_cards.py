...
def test_card_theme_variants_have_separate_cache_and_preserve_avatar(tmp_path, monkeypatch):
    import rep0rter.publishers.cards as cards
    from PIL import ImageColor, ImageFont

    avatar = io.BytesIO()
    Image.new('RGB', (82, 82), (219, 42, 91)).save(avatar, 'PNG')
    monkeypatch.setattr(cards, 'identity_image', lambda *_: avatar.getvalue())
    monkeypatch.setattr(cards, '_font', lambda _, size: ImageFont.load_default(size=size))
    monkeypatch.setattr(CardRenderer, '_page', lambda _: None)
    event = example()
    event.author_name, event.text = 'Test author', 'Original source message'
    with CardRenderer(Config(data_dir=tmp_path)) as renderer:
        for render in (lambda **kwargs: renderer.render(event, None, **kwargs),
                       lambda **kwargs: renderer.render_report(event, None, report_post(), **kwargs)):
            light, dark = render(), render(theme='dark')
            assert light != dark
            assert render(theme='light') == light
            for theme, path in (('light', light), ('dark', dark)):
                modified = path.stat().st_mtime_ns
                assert render(theme=theme) == path
                assert path.stat().st_mtime_ns == modified
                with Image.open(path) as image:
                    assert image.size == SIZE
                    assert image.getpixel((0, 0)) == ImageColor.getrgb(cards.CARD_PALETTES[theme]['canvas'])
                    assert image.getpixel((50, 300)) == ImageColor.getrgb(cards.CARD_PALETTES[theme]['surface'])
                    assert image.getpixel((100, 100)) == (219, 42, 91)


def test_html_theme_palette_and_real_avatar_are_preserved():
    from rep0rter.publishers.cards import CARD_PALETTES

    avatar = 'data:image/png;base64,unchanged-avatar'
    for template in ('card.html', 'report-card.html'):
        for theme, palette in CARD_PALETTES.items():
            markup = env.get_template(template).render(theme=theme, avatar=avatar)
            for value in palette.values():
                assert value in markup
            assert f'src="{avatar}"' in markup
            assert 'filter:' not in markup
        default = env.get_template(template).render()
        assert 'background:#eef2f6' in default


def test_invalid_card_theme_fails_before_identity_lookup(tmp_path, monkeypatch):
    import pytest
    import rep0rter.publishers.cards as cards

    def forbidden(*args):
        raise AssertionError('Invalid themes must fail before fetching identity images')

    monkeypatch.setattr(cards, 'identity_image', forbidden)
    with CardRenderer(Config(data_dir=tmp_path)) as renderer:
        with pytest.raises(ValueError, match='theme'):
            renderer.render(example(), None, theme='system')
        with pytest.raises(ValueError, match='theme'):
            renderer.render_report(example(), None, report_post(), theme='system')
